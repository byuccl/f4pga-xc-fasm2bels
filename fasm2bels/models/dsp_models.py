...
def process_dsp48e1_site(top, features, set_features):
    ...
    for bus, width in [("ALUMODE", 4), ("INMODE", 5), ("OPMODE", 7)]:
        key = f"IS_{bus}_INVERTED"
        for feat in features:
            if feat.feature.endswith(key):
                val = bin(feat.value)[2:].zfill(width)
        for idx, inv in zip(range(width), val):
            # This is not working correctly - no inversion shown in netlist
            site_wire = f"{bus}{idx}"
            site_pips = make_inverter_path(site_wire, not int(inv))
            site.add_sink(
                bel=bel,
                cell_pin=f"{bus}[{idx}]",
                sink_site_pin=site_wire,
    ...
    # for idx in range(30):
    #     bel.map_bel_pin_to_cell_pin(bel.bel, "ACIN{}".format(idx), "ACIN[{}]".format(idx))
    ...
    # for idx in range(18):
    #     bel.map_bel_pin_to_cell_pin(bel.bel, "BCIN{}".format(idx), "BCIN[{}]".format(idx))
    ...
    # for idx in range(48):
    #     bel.map_bel_pin_to_cell_pin(bel.bel, "PCIN{}".format(idx), "PCIN[{}]".format(idx))

    # bel.add_unconnected_port("PCOUT", 48, direction="output")
    # for idx in range(48):
    #     bel.map_bel_pin_to_cell_pin(bel.bel, "PCOUT{}".format(idx), "PCOUT[{}]".format(idx))
