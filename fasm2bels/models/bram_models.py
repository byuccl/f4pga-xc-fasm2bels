...
def clean_up_to_bram36(top, site):
    ...
    for idx in range(15):
        assert top.find_source_from_sink(
            site, "ADDRARDADDRL{}".format(idx)) == top.find_source_from_sink(
                site, "ADDRARDADDRU{}".format(idx))
        assert top.find_source_from_sink(
            site, "ADDRBWRADDRL{}".format(idx)) == top.find_source_from_sink(
                site, "ADDRBWRADDRU{}".format(idx))
    ...
    if bel.parameters["WRITE_WIDTH_A"] < 18:
        for idx in range(4):
            assert top.find_source_from_sink(
                site, "WEAL{}".format(idx)) == top.find_source_from_sink(
                    site, "WEAU{}".format(idx))
    ...
    if bel.parameters["WRITE_WIDTH_B"] < 18:
        for idx in range(8):
            assert top.find_source_from_sink(
                site, "WEBWEL{}".format(idx)) == top.find_source_from_sink(
                    site, "WEBWEU{}".format(idx))
...
def clean_brams(top, bram_sites, bram36_site, tile_features, verbose=False):
    ...
    if not eligible_for_merge(top, bram_sites, tile_features, verbose=verbose):
        ...
        try:
            ...
            print("Assertion err triggered during BRAM36. Attempting BRAM18")
            for bram in bram_sites:
                clean_up_to_bram18(top, bram)
            for sink, v in bram36_site.sinks.items():
                if None in v:
                    v.remove(None)
            top.remove_site(bram36_site)
...
def process_bram(conn, top, tile, features):
    ...
    try:
        ...
        print("Wanring disregarding RAM_EXTENSTION_A/B feature check ")
